...
public class WalkerState {
	...
	/**
	 * Creates a new WalkerState representing the start of the next hour from the
	 * current time
	 */
	public WalkerState() {
		this(new Date());
	 ...
	 * 
	 * @param direction  the direction to seek: two possibilities '<' go backward
	 *                   '>' go forward
	 * 
	 * @param seekType   the type of seek to perform (by_day or by_week) by_day
	 *                   means we seek to the very next occurrence of the given day
	 *                   by_week means we seek to the first occurrence of the given
	 *                   day week in the next (or previous,) week (or multiple of
	 *                   next or previous week depending on the seek amount.)
	 * 
	 * @param seekAmount the amount to seek. Must be guaranteed to parse as an
	 *                   integer
	 * 
	 * @param dayOfWeek  the day of the week to seek to, represented as an integer
	 *                   from 1 to 7 (1 being Sunday, 7 being Saturday.) Must be
	 *                   guaranteed to parse as an Integer
	 */
	public void seekToDayOfWeek(String direction, String seekType, String seekAmount, String dayOfWeek) {
		...
		if (seekType.equals(SEEK_BY_WEEK)) {
			...
			// first bug found the day was incorrect when using last/next.
	 ...
	 *                  from 1 to 366. Must be guaranteed to parse as an Integer. If
	 *                  this day is beyond the last day of the current year, the
	 *                  actual last day of the year will be used.
	 */
	 ...
	 *                   integer
	 * 
	 * @param month      the month to seek to. Must be guaranteed to parse as an
	...
	public void seekToMonth(String direction, String seekAmount, String month) {
		...
		// set the day to the first of month. This step is necessary because if we seek
		// to the
		// current day of a month whose number of days is less than the current day, we
		// will
		...
		if (seekAmountInt > 0) {
			...
			int sign = direction.equals(DIR_RIGHT) ? 1 : -1;
			int numYearsToShift = seekAmountInt + (currentMonth == monthInt ? 0
	...
	public void seekBySpan(String direction, String seekAmount, String span) {
		...
		int seekAmountInt = Integer.parseInt(seekAmount);
		assert (direction.equals(DIR_LEFT) || direction.equals(DIR_RIGHT));
		assert (span.equals(DAY) || span.equals(WEEK) || span.equals(MONTH) || span.equals(YEAR) || span.equals(HOUR)
				|| span.equals(MINUTE) || span.equals(SECOND));

		boolean isDateSeek = span.equals(DAY) || span.equals(WEEK) || span.equals(MONTH) || span.equals(YEAR);
		...
		int sign = direction.equals(DIR_RIGHT) ? 1 : -1;
		int field = span.equals(DAY) ? Calendar.DAY_OF_YEAR
								...
								: span.equals(YEAR) ? Calendar.YEAR
										: span.equals(HOUR) ? Calendar.HOUR
												: span.equals(MINUTE) ? Calendar.MINUTE
														: span.equals(SECOND) ? Calendar.SECOND : null;
		if (field > 0)
	 ...
	 *                   does not fall on the given day of week, we walk backwards
	 *                   in 1 year iterations until we find the first such date. If
	 *                   given, must be guaranteed to parse as an integer between 1
	 *                   and 7.
	 * 
	 * @param year       the year to set (optional). If present, must be guaranteed
	 *                   to parse as an integer between 0 and 9999
	 */
	 ...
	 * 
	 * @param hours      the hours to set. Must be guaranteed to parse as an integer
	 *                   between 0 and 23
	 * 
	 * @param minutes    the minutes to set. Must be guaranteed to parse as an
	 ...
	 * 
	 * @param zoneString the time zone to use in one of two formats: - zoneinfo
	 *                   format (America/New_York, America/Los_Angeles, etc) - GMT
	 *                   offset (+05:00, -0500, +5, etc)
	 */
	public void setExplicitTime(String hours, String minutes, String seconds, String amPm, String zoneString) {
		...
		if (hoursInt <= 12) {
			int amPmInt = amPm == null ? (hoursInt >= 12 ? Calendar.PM : Calendar.AM)
	...
	public void captureDateTime() {
		...
		// The list of times found per capture should always be 1 larger than the number
		// of captures.
		...
		// if a time was given in this capture, we'll want to use the same time for any
		// previous
		...
		// similarly, if a meridian indicator was given in this capture, we'll want to
		// use the same
	...
	private void seekToIcsEvent(String icsFileName, String eventSummary, String direction, String seekAmount) {
		...
		Map<Integer, Date> dates = getDatesFromIcs(icsFileName, eventSummary, startYear, endYear);

		// grab the right one
		boolean hasPassed = cal.getTime().after(dates.get(currentYear));
		int targetYear = currentYear
				+ (forwards ? seekAmountInt + (hasPassed ? 0 : -1) : (seekAmountInt - (hasPassed ? 1 : 0)) * -1);
	...
	private void seekToIcsEventYear(String icsFileName, String yearString, String eventSummary) {
		...
		// if we couldn't find a date for the given year due to it falling outside of
		// the range of
		// years present in the ics file, we'll make an educated guess based on the
		// current year.
		// This is likely to represent the correct start date of the given season, but
		// is not
	...
	/**
	 * Finds and returns the date for the given event summary and year within the
	 * given ics file, or null if not present.
	 */
	...
	private Map<Integer, Date> getDatesFromIcs(String icsFileName, String eventSummary, int startYear, int endYear) {
